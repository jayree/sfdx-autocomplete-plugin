...
const create_1 = tslib_1.__importDefault(require("../../../src/commands/autocmplt/create"));
...
runtest('Create', () => {
    ...
    describe('private methods', () => {
        ...
        it('#genCmdWithDescription', () => {
            chai_1.expect(cmd.genCmdWithDescription(klass)).to.eq('"autocmplt\\:foo":"foo cmd for autocomplete testing"');
        });
        ...
        it('#bashCommandsList', () => {
            chai_1.expect(cmd.bashCommandsList).to.eq('autocmplt --skip-instructions\nautocmplt:foo --targetusername --bar --json');
        });
...
`);
        ...
        it('#genZshCmdFlagsSetter', () => {
            chai_1.expect(cmd.genZshCmdFlagsSetter(cacheBuildFlagsTest)).to.eq(`_sfdx_set_autocmplt_create_flags () {
_sfdx_flags=(
"--targetusername=-[(autocomplete) targetusername to use]: :_sfdx_compadd_flag_options"
